/**
 * Contact Hero layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'contact_hero';

    var FIELD_KEYS = {
        heading: 'field_luux_contact_hero_heading',
        media_type: 'field_luux_contact_hero_media_type',
        background_image: 'field_luux_contact_hero_background_image',
        background_video: 'field_luux_contact_hero_background_video',
        section_id: 'field_luux_contact_hero_section_id',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutContactHero !== 'undefined' && luuxLayoutContactHero.ajaxurl) {
            return luuxLayoutContactHero.ajaxurl;
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-contact-hero-early').remove();

        var $wrap = $('<div id="luux-contact-hero-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                $('<input>', {
                    type: 'hidden',
                    name: 'luux_contact_hero[' + item.domIndex + '][' + name + ']',
                    value: value,
                }).appendTo($wrap);
            });
        });
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_contact_hero_fields',
            nonce: luuxLayoutContactHero.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_contact_hero';
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutContactHero === 'undefined') {
            return;
        }
