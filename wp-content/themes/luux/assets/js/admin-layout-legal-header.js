/**
 * Legal Header layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'legal_header';

    var FIELD_KEYS = {
        heading: 'field_luux_legal_header_heading',
        intro: 'field_luux_legal_header_intro',
        section_id: 'field_luux_legal_header_section_id',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutLegalHeader !== 'undefined' && luuxLayoutLegalHeader.ajaxurl) {
            return luuxLayoutLegalHeader.ajaxurl;
        }
    ...
    function readRowFields($layout) {
        ...
        $.each(FIELD_KEYS, function (name, key) {
            ...
            var $input = $fieldEl.find('input[type="text"]').first();
    ...
    function injectEarlyFields() {
        ...
        $('#luux-legal-header-early').remove();

        var $wrap = $('<div id="luux-legal-header-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name === 'intro') {
                    $('<textarea>', {
                        name: 'luux_legal_header[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_legal_header_fields',
            nonce: luuxLayoutLegalHeader.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_legal_header';
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutLegalHeader === 'undefined') {
            return;
        }
