/**
 * Legal Section layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'legal_section';

    var FIELD_KEYS = {
        heading: 'field_luux_legal_section_heading',
        intro: 'field_luux_legal_section_intro',
        section_id: 'field_luux_legal_section_section_id',
    };

    var CLAUSE_SUB_KEYS = {
        title: 'field_luux_legal_section_clause_title',
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutLegalSection !== 'undefined' && luuxLayoutLegalSection.ajaxurl) {
            return luuxLayoutLegalSection.ajaxurl;
        }
    ...
    function readWysiwyg($fieldEl) {
        ...
        var $fallback = $fieldEl.find('textarea').first();
    ...
    function readScalarFields($layout) {
        ...
        if (typeof acf !== 'undefined' && typeof acf.getField === 'function') {
            ...
            $rows.each(function () {
                ...
                $.each(CLAUSE_SUB_KEYS, function (name, key) {
                    ...
                    var value = name === 'body' ? readWysiwyg($fieldEl) : '';

                    if (name !== 'body' && typeof acf.getField === 'function') {
                        var field = acf.getField($fieldEl);
        ...
        Object.keys(bucket).sort(function (a, b) {
            ...
            if (raw.title) {
                clause.title = String(raw.title);
                clause.field_luux_legal_section_clause_title = clause.title;
            }
    ...
    function readRowFields($layout) {
        ...
        var clauses = readClauses($layout);

        if (clauses.length) {
            fields.clauses_json = JSON.stringify(clauses);
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-legal-section-early').remove();

        var $wrap = $('<div id="luux-legal-section-early" style="display:none" aria-hidden="true"></div>');

        getLayoutRows().forEach(function (item) {
            var fields = readRowFields(item.$el);

            $.each(fields, function (name, value) {
                if (name === 'clauses_json' || name === 'intro') {
                    $('<textarea>', {
                        name: 'luux_legal_section[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_legal_section_fields',
            nonce: luuxLayoutLegalSection.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_legal_section';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.clauses_json) {
            try {
                var clauses = JSON.parse(fields.clauses_json);

                if ($.isArray(clauses) && clauses.length) {
                    acfRow.field_luux_legal_section_clauses = clauses;
                    acfRow.clauses = clauses;
                }
            } catch (e) {
                // Ignore.
            }
    ...
    function isLayoutField(field) {
        ...
        return key.indexOf('field_luux_legal_section_') === 0;
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutLegalSection === 'undefined') {
            return;
        }
