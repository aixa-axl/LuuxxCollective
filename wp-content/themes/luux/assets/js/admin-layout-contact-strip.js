/**
 * Contact Strip layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'contact_strip';

    var FIELD_KEYS = {
        caption: 'field_luux_contact_strip_caption',
        text: 'field_luux_contact_strip_text',
        primary_link: 'field_luux_contact_strip_primary_link',
        secondary_link: 'field_luux_contact_strip_secondary_link',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutContactStrip !== 'undefined' && luuxLayoutContactStrip.ajaxurl) {
            return luuxLayoutContactStrip.ajaxurl;
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-contact-strip-early').remove();

        var $wrap = $('<div id="luux-contact-strip-early" style="display:none" aria-hidden="true"></div>');
        ...
        rows.forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name.indexOf('_json') !== -1) {
                    $('<textarea>', {
                        name: 'luux_contact_strip[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_contact_strip_fields',
            nonce: luuxLayoutContactStrip.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_contact_strip';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.caption) {
            acfRow.field_luux_contact_strip_caption = fields.caption;
            acfRow.caption = fields.caption;
        }

        if (fields.text) {
            acfRow.field_luux_contact_strip_text = fields.text;
    ...
    function init() {
        if (initialized || typeof luuxLayoutContactStrip === 'undefined') {
            return;
        }
