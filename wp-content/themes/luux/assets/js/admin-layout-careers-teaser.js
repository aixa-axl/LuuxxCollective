/**
 * Careers Teaser layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'careers_teaser';

    var FIELD_KEYS = {
        heading: 'field_luux_careers_teaser_heading',
        text: 'field_luux_careers_teaser_text',
        cta: 'field_luux_careers_teaser_cta',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutCareersTeaser !== 'undefined' && luuxLayoutCareersTeaser.ajaxurl) {
            return luuxLayoutCareersTeaser.ajaxurl;
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-careers-teaser-early').remove();

        var $wrap = $('<div id="luux-careers-teaser-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name.indexOf('_json') !== -1) {
                    $('<textarea>', {
                        name: 'luux_careers_teaser[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_careers_teaser_fields',
            nonce: luuxLayoutCareersTeaser.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
        ...
        $.each(fields, function (name, value) {
            if (name === 'cta_json' || name === 'heading' || name === 'text') {
                payload[name] = value;
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_careers_teaser';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.heading) {
            acfRow.field_luux_careers_teaser_heading = fields.heading;
            acfRow.heading = fields.heading;
        }

        if (fields.text) {
            acfRow.field_luux_careers_teaser_text = fields.text;
        ...
        if (fields.cta_json) {
            try {
                ...
                if (link && link.url) {
                    acfRow.field_luux_careers_teaser_cta = link;
                    acfRow.cta = link;
                }
            } catch (e) {
                // Ignore.
    ...
    function init() {
        if (initialized || typeof luuxLayoutCareersTeaser === 'undefined') {
            return;
        }
