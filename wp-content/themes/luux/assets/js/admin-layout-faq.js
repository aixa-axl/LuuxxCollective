...
(function ($) {
    ...
    var FIELD_KEYS = {
        heading: 'field_luux_faq_heading',
        section_id: 'field_luux_faq_section_id',
    };

    var ITEM_SUB_KEYS = {
        question: 'field_luux_faq_question',
        answer: 'field_luux_faq_answer',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutFaq !== 'undefined' && luuxLayoutFaq.ajaxurl) {
            return luuxLayoutFaq.ajaxurl;
        }
    ...
    function readItems($layout) {
        var items = [];
        var $repeater = $layout.find('.acf-field[data-key="field_luux_faq_items"], .acf-field[data-name="items"]').first();
        ...
        if (typeof acf !== 'undefined' && typeof acf.getField === 'function') {
            ...
            $rows.each(function () {
                ...
                $.each(ITEM_SUB_KEYS, function (name, key) {
                    ...
                    item['field_luux_faq_' + name] = item[name];
                });
        ...
        var bucket = {};
        var pattern = /\[(?:field_luux_faq_items|items)\]\[(?:row-)?(\d+)\]\[(?:field_luux_faq_)?(question|answer)\]/;
        ...
        Object.keys(bucket).sort(function (a, b) {
            ...
            if (raw.question) {
                item.question = String(raw.question);
                item.field_luux_faq_question = item.question;
            }

            if (raw.answer) {
                item.answer = String(raw.answer);
                item.field_luux_faq_answer = item.answer;
            }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-faq-early').remove();

        var $wrap = $('<div id="luux-faq-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name === 'items_json') {
                    $('<textarea>', {
                        name: 'luux_faq[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_faq_fields',
            nonce: luuxLayoutFaq.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_faq';
    }
        ...
        if (fields.items_json) {
            try {
                ...
                if ($.isArray(items) && items.length) {
                    acfRow.field_luux_faq_items = items;
                    acfRow.items = items;
                }
            } catch (e) {
                // Ignore.
    ...
    function isLayoutField(field) {
        ...
        return key.indexOf('field_luux_faq_') === 0;
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutFaq === 'undefined') {
            return;
        }
