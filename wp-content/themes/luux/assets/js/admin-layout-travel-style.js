/**
 * Travel Style layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'travel_style';

    var FIELD_KEYS = {
        section_label: 'field_luux_travel_style_section_label',
        heading: 'field_luux_travel_style_heading',
        footer_heading: 'field_luux_travel_style_footer_heading',
        cta: 'field_luux_travel_style_cta',
    };

    var CATEGORY_SUB_KEYS = {
        image: 'field_luux_travel_style_image',
        title: 'field_luux_travel_style_title',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutTravelStyle !== 'undefined' && luuxLayoutTravelStyle.ajaxurl) {
            return luuxLayoutTravelStyle.ajaxurl;
        }
    ...
    function readCta($layout) {
        var $fieldEl = $layout.find('.acf-field[data-key="' + FIELD_KEYS.cta + '"]').first();
        ...
        if (typeof acf !== 'undefined' && typeof acf.getField === 'function') {
            ...
            if (field && typeof field.val === 'function') {
                var link = normalizeLink(field.val());
        ...
        $fieldEl.find('input').each(function () {
            ...
            if (/\[url\]$/.test(name)) {
                raw.url = String(value);
            } else if (/\[title\]$/.test(name)) {
                raw.title = String(value);
            } else if (/\[target\]$/.test(name)) {
                raw.target = String(value);
            }
        });
    ...
    function readCategories($layout) {
        var categories = [];
        var $repeater = $layout.find('.acf-field[data-key="field_luux_travel_style_categories"], .acf-field[data-name="categories"]').first();
                ...
                $.each(CATEGORY_SUB_KEYS, function (name, key) {
                    ...
                    if (name === 'image') {
                        ...
                        if (imageId !== '') {
                            ...
                            item.field_luux_travel_style_image = item.image;
                        }
                    ...
                    item['field_luux_travel_style_' + name] = item[name];
                });
        ...
        // Fallback: parse raw inputs.
        var bucket = {};
        var pattern = /\[(?:field_luux_travel_style_categories|categories)\]\[(?:row-)?(\d+)\]\[(?:field_luux_travel_style_)?(image|title)\]/;
        ...
        Object.keys(bucket).sort(function (a, b) {
            ...
            if (raw.title) {
                ...
                item.field_luux_travel_style_title = item.title;
            }
    ...
    function readRowFields($layout) {
        ...
        if (cta) {
            fields.cta_json = JSON.stringify(cta);
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-travel-style-early').remove();

        var $wrap = $('<div id="luux-travel-style-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name.indexOf('_json') !== -1) {
                    $('<textarea>', {
                        name: 'luux_travel_style[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_travel_style_fields',
            nonce: luuxLayoutTravelStyle.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
        ...
        $.each(fields, function (name, value) {
            if (name === 'categories_json' || name === 'cta_json') {
                payload[name] = value;
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_travel_style';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.categories_json) {
            try {
                var categories = JSON.parse(fields.categories_json);

                if ($.isArray(categories) && categories.length) {
                    acfRow.field_luux_travel_style_categories = categories;
        ...
        if (fields.cta_json) {
            try {
                var cta = JSON.parse(fields.cta_json);

                if (cta && cta.url) {
                    acfRow.field_luux_travel_style_cta = cta;
                    acfRow.cta = cta;
                }
            } catch (e) {
                // Ignore.
    ...
    function isLayoutField(field) {
        ...
        return key.indexOf('field_luux_travel_style_') === 0;
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutTravelStyle === 'undefined') {
            return;
        }
