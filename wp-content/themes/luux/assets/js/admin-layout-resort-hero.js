/**
 * Resort Hero layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'resort_hero';

    var FIELD_KEYS = {
        eyebrow: 'field_luux_resort_hero_eyebrow',
        heading: 'field_luux_resort_hero_heading',
        text: 'field_luux_resort_hero_text',
        media_type: 'field_luux_resort_hero_media_type',
        background_image: 'field_luux_resort_hero_background_image',
        background_video: 'field_luux_resort_hero_background_video',
        cta: 'field_luux_resort_hero_cta',
        section_id: 'field_luux_resort_hero_section_id',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutResortHero !== 'undefined' && luuxLayoutResortHero.ajaxurl) {
            return luuxLayoutResortHero.ajaxurl;
        }
    ...
    function readRowFields($layout) {
        ...
        $.each(FIELD_KEYS, function (name, key) {
            ...
            if (name === 'cta') {
                ...
                if (link) {
                    fields.cta_json = JSON.stringify(link);
                }
            ...
            if (name === 'background_image' || name === 'background_video') {
    ...
    function injectEarlyFields() {
        ...
        $('#luux-resort-hero-early').remove();

        var $wrap = $('<div id="luux-resort-hero-early" style="display:none" aria-hidden="true"></div>');

        getLayoutRows().forEach(function (item) {
            var fields = readRowFields(item.$el);

            $.each(fields, function (name, value) {
                if (name === 'cta_json' || name === 'text') {
                    $('<textarea>', {
                        name: 'luux_resort_hero[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_resort_hero_fields',
            nonce: luuxLayoutResortHero.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_resort_hero';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.cta_json) {
            try {
                var link = JSON.parse(fields.cta_json);

                if (link && link.url) {
                    acfRow.field_luux_resort_hero_cta = link;
                    acfRow.cta = link;
                }
            } catch (e) {
                // Ignore.
    ...
    function init() {
        if (initialized || typeof luuxLayoutResortHero === 'undefined') {
            return;
        }
