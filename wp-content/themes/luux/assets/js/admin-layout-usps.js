/**
 * USPs layout admin save helpers.
 */
(function ($) {
    'use strict';
    ...
    var FIELD_KEYS = {
        section_label: 'field_luux_usps_section_label',
        heading: 'field_luux_usps_heading',
        light_background: 'field_luux_usps_light_background',
        section_id: 'field_luux_usps_section_id',
    };

    var ITEM_SUB_KEYS = {
        icon: 'field_luux_usps_icon',
        title: 'field_luux_usps_title',
        description: 'field_luux_usps_description',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutUsps !== 'undefined' && luuxLayoutUsps.ajaxurl) {
            return luuxLayoutUsps.ajaxurl;
        }
    ...
    function readItems($layout) {
        var items = [];
        var $repeater = $layout.find('.acf-field[data-key="field_luux_usps_items"], .acf-field[data-name="items"]').first();
        ...
        if (typeof acf !== 'undefined' && typeof acf.getField === 'function') {
            ...
            $rows.each(function () {
                ...
                $.each(ITEM_SUB_KEYS, function (name, key) {
                    ...
                    if (name === 'icon') {
                        var iconId = normalizeAttachmentId(value);

                        if (iconId !== '') {
                            item.icon = parseInt(iconId, 10);
                            item.field_luux_usps_icon = item.icon;
                        }
                    ...
                    item['field_luux_usps_' + name] = item[name];
                });
        ...
        var bucket = {};
        var pattern = /\[(?:field_luux_usps_items|items)\]\[(?:row-)?(\d+)\]\[(?:field_luux_usps_)?(icon|title|description)\]/;
        ...
        Object.keys(bucket).sort(function (a, b) {
            ...
            if (raw.icon) {
                var iconId = normalizeAttachmentId(raw.icon);
            ...
            if (raw.title) {
                ...
                item.field_luux_usps_title = item.title;
            }

            if (raw.description) {
                item.description = String(raw.description);
                item.field_luux_usps_description = item.description;
            }
    ...
    function readRowFields($layout) {
        ...
        var items = readItems($layout);

        if (items.length) {
            fields.items_json = JSON.stringify(items);
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-usps-early').remove();

        var $wrap = $('<div id="luux-usps-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name === 'items_json') {
                    $('<textarea>', {
                        name: 'luux_usps[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_usps_fields',
            nonce: luuxLayoutUsps.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_usps';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.items_json) {
            try {
                var items = JSON.parse(fields.items_json);

                if ($.isArray(items) && items.length) {
                    acfRow.field_luux_usps_items = items;
                    acfRow.items = items;
                }
            } catch (e) {
                // Ignore.
    ...
    function isLayoutField(field) {
        ...
        return key.indexOf('field_luux_usps_') === 0;
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutUsps === 'undefined') {
            return;
        }
