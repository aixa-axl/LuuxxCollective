/**
 * Trust Strip layout admin save helpers.
 */
(function ($) {
    'use strict';

    var LAYOUT = 'trust_strip';

    var FIELD_KEYS = {
        rating: 'field_luux_trust_strip_rating',
        award: 'field_luux_trust_strip_award',
    };

    var LOGO_SUB_KEYS = {
        name: 'field_luux_trust_strip_press_name',
    };
    ...
    function getAjaxUrl() {
        if (typeof luuxLayoutTrustStrip !== 'undefined' && luuxLayoutTrustStrip.ajaxurl) {
            return luuxLayoutTrustStrip.ajaxurl;
        }
    ...
    function readPressLogos($layout) {
        var logos = [];
        var $repeater = $layout.find('.acf-field[data-key="field_luux_trust_strip_press"], .acf-field[data-name="press_logos"]').first();
        ...
        if (typeof acf !== 'undefined' && typeof acf.getField === 'function') {
            ...
            $rows.each(function () {
                ...
                $.each(LOGO_SUB_KEYS, function (name, key) {
                    ...
                    logo.name = String(value);
                    logo.field_luux_trust_strip_press_name = logo.name;
                });
        ...
        var bucket = {};
        var pattern = /\[(?:field_luux_trust_strip_press|press_logos)\]\[(?:row-)?(\d+)\]\[(?:field_luux_trust_strip_press_)?(name)\]/;
        ...
        Object.keys(bucket).sort(function (a, b) {
            ...
            if (raw.name) {
                logos.push({
                    name: raw.name,
                    field_luux_trust_strip_press_name: raw.name,
                });
            }
        });
    ...
    function readRowFields($layout) {
        ...
        var logos = readPressLogos($layout);

        if (logos.length) {
            fields.press_logos_json = JSON.stringify(logos);
        }
    ...
    function injectEarlyFields() {
        ...
        $('#luux-trust-strip-early').remove();

        var $wrap = $('<div id="luux-trust-strip-early" style="display:none" aria-hidden="true"></div>');
        ...
        getLayoutRows().forEach(function (item) {
            ...
            $.each(fields, function (name, value) {
                if (name === 'press_logos_json') {
                    $('<textarea>', {
                        name: 'luux_trust_strip[' + item.domIndex + '][' + name + ']',
                        'aria-hidden': 'true',
    ...
    function saveRowAjax(rowIndex, rowNth, fields, async) {
        ...
        var payload = {
            action: 'luux_save_trust_strip_fields',
            nonce: luuxLayoutTrustStrip.nonce,
            post_id: postId,
            row_index: rowIndex,
            row_nth: rowNth,
            fields: {},
        ...
        $.each(fields, function (name, value) {
            if (name === 'press_logos_json') {
                payload.press_logos_json = value;
    ...
    function layoutMatches(layout) {
        return layout === LAYOUT || layout === 'layout_luux_trust_strip';
    }
    ...
    function mergeRowIntoAcfPayload(fields, acfRow) {
        ...
        if (fields.press_logos_json) {
            try {
                var logos = JSON.parse(fields.press_logos_json);

                if ($.isArray(logos) && logos.length) {
                    acfRow.field_luux_trust_strip_press = logos;
                    acfRow.press_logos = logos;
                }
            } catch (e) {
                // Ignore.
            }
    ...
    function isLayoutField(field) {
        ...
        return key.indexOf('field_luux_trust_strip_') === 0;
    }
    ...
    function init() {
        if (initialized || typeof luuxLayoutTrustStrip === 'undefined') {
            return;
        }
