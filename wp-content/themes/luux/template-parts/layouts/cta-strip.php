...
<section class="bg-brand-dark p-10 lg:section-pad">
    <div class="container-site flex flex-col items-start gap-8 text-left lg:flex-row lg:items-center lg:justify-between lg:gap-6 lg:text-left">
        <?php if ($text) : ?>
            <p class="font-body text-body-lg text-brand-white lg:max-w-none"><?php echo esc_html($text); ?></p>
        <?php endif; ?>

        <?php if ($primary_link || $secondary_link) : ?>
            <div class="flex w-full flex-col gap-4 lg:w-auto lg:flex-row lg:gap-10">
                ...
                <?php if (! empty($secondary_link['url'])) : ?>
                    <a class="link-underline-block font-display text-body text-brand-cream lg:py-3"
